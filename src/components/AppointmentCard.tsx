...
const AppointmentCard = ({ token }: { token: string }) => {
  ...
  const handleDeleteWithErrorHandling = async (appointmentId: string | number) => {
    try {
      await handleDelete(appointmentId, refetch);
    } catch (error) {
  ...
  return (
    <div className="flex flex-wrap -m-4">
              ...
              {isEditing && editingAppointment?.id === appointment.id ? (
                ...
                <>
                  ...
                  <div className="flex items-center flex-wrap">
                    ...
                    <button className="bg-white hover:bg-gray-100 w-30 text-gray-800 font-semibold py-2 px-4 border border-gray-400 rounded shadow mr-4" onClick={() => handleDeleteWithErrorHandling(appointment.id)}>Eliminar
      ...
      {isErrorModalOpen && <ErrorModal show={isErrorModalOpen} handleClose={handleCloseErrorModal} />}
